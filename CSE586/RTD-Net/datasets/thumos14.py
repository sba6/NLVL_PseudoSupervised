...
class VideoRecord:
    def __init__(self, vid, num_frames, locations, gt, s_e_scores, fps, args):
        ...
        self.interval = float(locations[1]) - float(locations[0]) #args.interval
        self.locations_norm = [
        ...
        for i in self.gt_norm:
            ...
            if i[0][0] > 1 or i[0][1] < 0:
                ...
                #print(self.locations)
                #print(first, second)
                #print(i[0][0], i[0][1], self.base, self.interval, self.window_size)
        self.gt_s_e_frames = [i[0] for i in self.gt_norm]
        #print(self.gt_s_e_frames)
        self.fps = fps
...
class ThumosDetection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for vid in video_list:
            if (self.split=='val' and self.anno_file[vid]['subset']=='train') or (self.split=='test' and self.anno_file[vid]['subset']=='test') or self.split=='all':
    ...
    def get_data(self, video: VideoRecord):
        ...
        for (s, e, _) in gt_s_e_frames:
            if not (s >= 0 and s <= 1 and e >= 0 and e <= 1):
                print(s,e,vid, num_frames)
            assert s >= 0 and s <= 1 and e >= 0 and e <= 1, '{} {} {}'.format(s, e, vid)
        targets = {
            'labels': [],
            'boxes': [],
